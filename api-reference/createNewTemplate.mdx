...
- **Required**
  - `pdf` (file, binary): The template PDF or DOCX file to upload (Note: You can upload a DOCX file, it will be converted to a PDF).
...
async function createNewTemplate() {
  ...
  // Server expects field name 'pdf' (multer.single('pdf'))
  form.append('pdf', docxBlob, 'Important_Contract.docx');
